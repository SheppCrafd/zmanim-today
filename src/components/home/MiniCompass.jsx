...
export default function MiniCompass({ location }) {
    ...
    const bearing = location ? bearingTo(location.latitude, location.longitude) : 0;
    ...
    const tip = { x: cx + Math.sin(arrowRad) * tipDist, y: cy - Math.cos(arrowRad) * tipDist };
    const base = { x: cx + Math.sin(arrowRad) * baseDist, y: cy - Math.cos(arrowRad) * baseDist };
    const perp = arrowRad + Math.PI / 2;
    const wingBack = { x: cx + Math.sin(arrowRad) * (tipDist - 10), y: cy - Math.cos(arrowRad) * (tipDist - 10) };
    const lw = { x: wingBack.x + Math.sin(perp) * 6, y: wingBack.y - Math.cos(perp) * 6 };
    const rw = { x: wingBack.x - Math.sin(perp) * 6, y: wingBack.y + Math.cos(perp) * 6 };
        ...
        <Link to="/Compass" className="block">
            <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-4 hover:bg-slate-50 transition-colors active:bg-slate-100">
                <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="shrink-0">
                    ...
                    {/* Arrow */}
                    <line x1={base.x} y1={base.y} x2={wingBack.x} y2={wingBack.y} stroke="#d97706" strokeWidth="2" strokeLinecap="round" />
                    <polyline points={`${lw.x},${lw.y} ${tip.x},${tip.y} ${rw.x},${rw.y}`} fill="none" stroke="#d97706" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                ...
                <div className="min-w-0">
                    ...
                    <p className="text-amber-600 font-bold text-lg leading-tight">
                        {Math.round(bearing)}° <span className="text-sm font-medium">{bearingLabel(bearing)}</span>
                    </p>
                </div>
