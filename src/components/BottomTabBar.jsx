...
function TabButton({ isActive, onClick, icon: Icon, label }) {
  return (
    <button
      onClick={onClick}
      className="flex-1 min-w-0 flex flex-col items-center justify-center gap-1 py-2 active:scale-90 transition-transform"
    >
      <span
        className={`flex items-center justify-center w-9 h-7 rounded-full transition-colors ${isActive ? "bg-primary/10" : ""}`}
      >
        <Icon
...
// Primary navigation for every top-level ("tab") page. Deliberately laid out
// as a normal flex-column sibling (see the TabPage shell in App.jsx) rather
// than `position: fixed` — a fixed element's positioning depends on the
// nearest ancestor that establishes a containing block for it, which any
// wrapping context (an embedding iframe applying its own transform/zoom for
// a device-frame preview, e.g.) can silently redefine out from under it. A
// normal in-flow flex child has no such dependency: if its parent shell is
// visible, so is this.
//
// Not rendered inside the Siddur reading views themselves (SiddurView.jsx)
...
export default function BottomTabBar() {
  ...
  return (
    <>
      <nav
        className="shrink-0 bg-white/95 dark:bg-slate-950/95 backdrop-blur-sm border-t border-slate-200 dark:border-slate-800 flex items-stretch"
